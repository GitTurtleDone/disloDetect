...
export function useResetInputs() {
  ...
  useEffect(() => {
    if (shouldResetInputs) {
      ...
      Object.keys(customOutputInfos).forEach((key) => {
        const value = customOutputInfos[key];
        console.log(value["dftVal"]);
        document.getElementById(value["iptId"]).value =
          parseFloat(value["dftVal"]) < 1000
            ? parseFloat(value["dftVal"])
            : parseFloat(value["dftVal"]).toPrecision(2);
      });
      Object.keys(Infos[1]).forEach((key) => {
        const value = inputSliderInfos[key];
        // console.log("value slider: ", value["dftVal"]);
        document.getElementById(value["iptId"]).value = parseFloat(
          value["dftVal"],
        );
        document.getElementById(value["sldId"]).value = parseFloat(
          value["dftVal"],
        );
      });
      setShouldResetInputs(false);
    }
  }, [shouldResetInputs, customOutputInfos, inputSliderInfos]);
