...
namespace dislodetect_be.tests.ControllerTests
{
    public class PredictControllerTests
    {
        ...
        public async Task Predict_WithValidPhotoUrl_ReturnsOk()
        {
            ...
            _mockHandler.Setup(h => h.GetImageFromS3Async("https://example.com/image.jpg"))
        ...
        [Fact]
        public async Task Predict_WithS3Error_ReturnsBadRequest()
        {
            // Arrange
            var formCollection = new FormCollection(new Dictionary<string, Microsoft.Extensions.Primitives.StringValues>
            {
            ...
            // Assert
            var badRequestResult = Assert.IsType<BadRequestObjectResult>(result);
            Assert.Equal("S3 error", badRequestResult.Value);
        }

        [Fact]
        public async Task Predict_WithHandlerException_ReturnsBadRequest()
        {
            // Arrange - mock handler to throw exception and try catch block triggering an exception
            var formCollection = new FormCollection(new Dictionary<string, Microsoft.Extensions.Primitives.StringValues>
            {
                ["photoUrl"] = "https://example.com/image.jpg"
            });
